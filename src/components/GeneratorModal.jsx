...
const GeneratorModal = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-center items-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-zinc-900 w-full max-w-lg rounded-xl shadow-2xl p-8 border border-white/10 text-center relative" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-white mb-2">¿No sabéis qué hacer hoy?</h2>
        <p className="text-gray-400 mb-8">Deja que el algoritmo de nuestra relación decida el plan perfecto por vosotros.</p>

        {isGenerating && (
          <div className="py-8 flex flex-col items-center gap-4">
            <div className="w-12 h-12 border-4 border-white/10 border-t-red-600 rounded-full animate-spin" />
            <p className="text-gray-300 italic">{loadingText}</p>
            <div className="h-8 text-xl font-bold text-gray-500 overflow-hidden mt-4">
        ...
        {!isGenerating && selectedPlan && (
          <div className="bg-black/30 rounded-lg p-6 border-l-4 border-red-600 text-left mb-8 animate-in slide-in-from-bottom-4 fade-in">
            <div className="flex items-center gap-3 mb-2">
              {selectedPlan.icon}
              <span className="text-green-500 font-bold text-xs tracking-wider uppercase">¡Coincidencia Ideal!</span>
            </div>
            ...
            <div className="pt-4 border-t border-white/10 text-xs text-gray-400 flex flex-col gap-1">
        ...
        <button 
          ...
          className={`w-full py-4 rounded font-bold text-lg flex items-center justify-center gap-2 transition-all ${
