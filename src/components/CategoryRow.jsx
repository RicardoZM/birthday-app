...
const CategoryRow = ({ category, onOpenDetails }) => {
  ...
  const handleScroll = (direction) => {
    if (rowRef.current) {
      ...
      rowRef.current.scrollTo({ left: rowRef.current.scrollLeft + (scrollAmount * direction), behavior: 'smooth' });
    }
  };

  return (
    <div className="mb-10 relative group px-4 md:px-12" id={category.id}>
      <h2 className="text-xl md:text-2xl font-bold text-gray-200 mb-4 hover:text-white cursor-pointer transition-colors flex items-center gap-2">
        ...
        <div ref={rowRef} className="flex gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-hide py-4 -my-4 px-1">
          {category.items.map(item => (
            <div 
              ...
              className="relative flex-none w-[200px] md:w-[260px] aspect-video rounded-md overflow-hidden cursor-pointer transition-all duration-300 hover:scale-110 hover:z-30 hover:shadow-2xl hover:shadow-black/80 snap-center"
            >
              <img src={item.image} alt={item.title} className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/20 to-transparent opacity-0 hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
